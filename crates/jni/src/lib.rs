...
async fn run_mcs(
   ...
   let mut last_received = senders
      .keys()
      .map(|app_id| (app_id.clone(), now))
      .collect::<HashMap<_, _>>();
   let mut heartbeat =
      tokio::time::interval_at(now + MCS_HEARTBEAT_INTERVAL, MCS_HEARTBEAT_INTERVAL);
   heartbeat.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
   let idle_deadline = tokio::time::sleep(MCS_IDLE_TIMEOUT);
   ...
   loop {
      let event = tokio::select! {
         ...
         _ = &mut idle_deadline => {
            let now = tokio::time::Instant::now();
            let idle_app = last_received
               .iter()
               .find_map(|(app_id, last)| {
                  (now.duration_since(*last) >= MCS_IDLE_TIMEOUT).then_some(app_id)
               })
               .ok_or_else(|| NativeError("MCS idle deadline fired early".into()))?;
            return Err(NativeError(format!(
               "MCS connection for {idle_app} exceeded the idle deadline"
            )));
         },
         _ = heartbeat.tick() => {
            for (app_id, stream) in streams.iter_mut() {
               let stream = stream.stream_mut()?;
               let ping = new_heartbeat_ping(stream.last_stream_id_received());
               if ping.is_empty() {
                  return Err(NativeError(format!(
                     "failed to serialize MCS heartbeat ping for {app_id}"
                  )));
               }
               write_all_with_deadline(&mut **stream, &ping).await.map_err(|error| {
                  NativeError(format!("failed to send MCS heartbeat ping for {app_id}: {error}"))
               })?;
            }
            continue;
         },
      ...
      let now = tokio::time::Instant::now();
      last_received.insert(app_id.clone(), now);
      let next_idle_deadline = last_received
         .values()
         .map(|last| *last + MCS_IDLE_TIMEOUT)
         .min()
         .ok_or_else(|| NativeError("no MCS idle deadlines remain".into()))?;
      idle_deadline.as_mut().reset(next_idle_deadline);
      let stream = streams
         .iter_mut()
      ...
      match message {
         Message::Data(data) => {
            ...
            write_all_with_deadline(&mut **stream, &ack)
               .await
               .map_err(|error| {
         ...
         // This crate has no log channel, so a rejected login surfaces as the
         // run error the Kotlin side already records.
         Message::Other(tag, body) => {
            if tag == MessageTag::LoginResponse as u8
               && let Ok(response) = decode_login_response(&body)
               && response.error_code.is_some()
            {
               return Err(NativeError(format!(
                  "MCS login rejected for {app_id}: error_code={:?}, error_message={:?}, \
                   error_type={:?}",
                  response.error_code, response.error_message, response.error_type,
               )));
            }
         },
      }
