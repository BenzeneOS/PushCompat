...
impl SocketHub {
   ...
   pub(crate) fn attach(
      ...
      if let Some(previous) = attachments.insert(install_id.to_owned(), AttachedSocket {
         connection_id,
         sender: sender.clone(),
      }) && previous.connection_id != connection_id
      {
         let _ = previous
            .sender
            .send(SocketEvent::Replaced(install_id.to_owned()));
      }
   }
...
pub async fn upgrade(
   ...
   let hub = state.socket_hub;
   let (response, future) = ws.upgrade().map_err(|error| invalid_upgrade(&error))?;
...
fn invalid_upgrade(error: &WebSocketError) -> (StatusCode, String) {
   ...
   (
      StatusCode::BAD_REQUEST,
      "invalid WebSocket upgrade".to_owned(),
   )
}
