...
fn decode_key_param(b64: &str) -> Option<[u8; 65]> {
   ...
   let arr = <[u8; 65]>::try_from(bytes).ok()?;
   (arr[0] == 0x04).then_some(arr)
}
...
pub mod test_support {
   ...
   pub fn signed_header(aud: &str, exp: u64) -> (String, String) {
      let signing = SigningKey::from_slice(&[11_u8; 32]).unwrap();
...
mod tests {
   ...
   fn keypair() -> (SigningKey, String) {
      let signing = SigningKey::from_slice(&[7_u8; 32]).unwrap();
