...
impl V2Connection {
   ...
   async fn acknowledge(&mut self, install_id: &InstallId, id: MessageId) -> Result<bool> {
      ...
      if !matches!(
         attachment.state,
         AttachmentState::AwaitingAck { id: expected, .. } if expected == id
      ) {
         return Ok(false);
      }
   ...
   async fn negative_acknowledge(
      ...
      let Some(attachment) = self.attachments.get(install_id) else {
         return Ok(NackOutcome::Invalid);
      };
      let AttachmentState::AwaitingAck {
         id: expected,
         cursor,
         ref app_id,
         attempts,
      ...
      let failed_deliveries = attempts + 1;
      let outcome = if failed_deliveries >= MAX_DELIVERY_ATTEMPTS {
         self.db.delete_outbox_message(id).await?;
         warn!(
            "Discarded WebSocket v2 message {id} for {install_id}/{app_id} after \
             {failed_deliveries} failed deliveries"
         );
         NackOutcome::Discarded
      } else {
         let retry_seconds = 5_u32 << attempts.min(6);
         self
            .db
            .defer_outbox_message(id, i64::from(retry_seconds))
            .await?;
         warn!(
            "Deferred WebSocket v2 message {id} for {install_id}/{app_id} by {retry_seconds}s \
             after failed delivery {failed_deliveries}"
         );
         let hub = Arc::clone(&self.hub);
         let install = install_id.as_ref().to_owned();
         tokio::spawn(async move {
            sleep(Duration::from_secs(u64::from(retry_seconds) + 1)).await;
            hub.wake(&install);
         });
         NackOutcome::Deferred
      };
      let attachment = self
         .attachments
         .get_mut(install_id)
         .expect("attachment vanished while acknowledging");
      attachment.state = AttachmentState::Ready(cursor);
      Ok(outcome)
   }
   ...
   async fn pending_messages(&mut self) -> Result<Vec<ServerFrame>> {
      ...
      for install_id in install_ids {
         ...
         if let Some(attachment) = self.attachments.get_mut(&install_id) {
            attachment.state = AttachmentState::AwaitingAck {
               id: message_id,
               cursor,
   ...
   fn shutdown(&mut self) {
      let install_ids = self.attachments.keys().cloned().collect::<Vec<InstallId>>();
      for install_id in install_ids {
...
pub async fn run_socket(upgrade: UpgradeFut, db: Arc<Database>, hub: Arc<SocketHub>) {
   let mut socket = match upgrade.await {
      Ok(socket) => socket,
      Err(error) => {
         warn!("WebSocket v2 upgrade failed: {error}");
         return;
      },
   };
   ...
   'connection: loop {
      let pending = match connection.pending_messages().await {
         ...
         Err(error) => {
            error!(
               "Failed to load WebSocket v2 outbox for connection {}: {error}",
               connection.connection_id
            );
            break;
         },
              ...
              match incoming {
                  Ok(frame) if frame.opcode == OpCode::Text => {
                      let frame = match serde_json::from_slice::<ClientFrame>(&frame.payload) {
                          ...
                          Err(error) => {
                              warn!(
                                  "Invalid WebSocket v2 message on connection {}: {error}",
                                  connection.connection_id
                              );
                              break;
                          }
                      ...
                      match frame {
                          ClientFrame::Attach {
                              ...
                              match attached {
                                  ...
                                  Err(error) => {
                                      error!(
                                          "WebSocket v2 attach failed on connection {}: {error}",
                                          connection.connection_id
                                      );
                                      break;
                                  }
                          ...
                          ClientFrame::Ack { install_id, id } => {
                              ...
                              match acknowledged {
                                  ...
                                  Err(error) => {
                                      error!(
                                          "WebSocket v2 ack failed for {install_id}/{id}: {error}"
                                      );
                                      break;
                                  }
                              }
                          }
                          ClientFrame::Nack { install_id, id } => {
                              let parsed = InstallId::try_from(install_id.as_str());
                              let outcome = match &parsed {
                                  Ok(parsed) => {
                                      connection.negative_acknowledge(parsed, id).await
                                  }
                                  Err(_) => Ok(NackOutcome::Invalid),
                              };
                              match outcome {
                                  Ok(NackOutcome::Deferred | NackOutcome::Discarded) => {}
                                  Ok(NackOutcome::Invalid) => {
                                      warn!(
                                          "Invalid WebSocket v2 nack for {install_id}/{id}"
                                      );
                                      if let Ok(parsed) = &parsed {
                                          connection.detach(parsed);
                                      }
                                  ...
                                  Err(error) => {
                                      error!(
                                          "WebSocket v2 nack failed for {install_id}/{id}: {error}"
                                      );
                                      break;
                                  }
                              }
...
fn message_aad(install_id: &InstallId, id: MessageId) -> Vec<u8> {
   let mut aad = Vec::with_capacity(install_id.as_ref().len() + 1 + size_of::<i64>());
...
mod tests {
   ...
   async fn fresh_connection() -> (Arc<Database>, Arc<SocketHub>, V2Connection) {
      ...
      let (connection, _events) =
         V2Connection::with_nonce(Arc::clone(&db), Arc::clone(&hub), [7_u8; NONCE_BYTES]);
      (db, hub, connection)
   }
   ...
   fn payload_crypto_matches_cross_language_vector() {
      ...
      let message = OutboxMessage {
         ...
         app_id:          AppId::trusted("com.example.app"),
   ...
   async fn two_attached_installs_route_only_their_encrypted_payloads() {
      ...
      let app_a = AppId::trusted("com.example.a");
      let app_b = AppId::trusted("com.example.b");
      claim(&db, &install_a, &InstallSecret::from(SECRET_A)).await;
      claim(&db, &install_b, &InstallSecret::from(SECRET_B)).await;
      db.enqueue_fcm_message(&install_a, &app_a, Some("a-1"), b"payload-a")
         .await
         .unwrap();
      db.enqueue_fcm_message(&install_b, &app_b, Some("b-1"), b"payload-b")
         .await
         .unwrap();
      ...
      for frame in frames {
         let ServerFrame::Message {
            ...
            unreachable!("expected message frame");
         };
   ...
   async fn detach_leaves_outbox_for_cursor_replay_on_reattach() {
      let (db, hub, mut connection) = fresh_connection().await;
      let install_id = InstallId::try_from(INSTALL_A).unwrap();
      let app_id = AppId::trusted("com.example.a");
      ...
      let message_id = db
         .enqueue_fcm_message(&install_id, &app_id, Some("a-offline"), b"offline")
         .await
         .unwrap()
         .unwrap();
   ...
   #[tokio::test]
   async fn nack_defers_then_redelivers_the_same_message() {
      let (db, _hub, mut connection) = fresh_connection().await;
      let install_id = InstallId::try_from(INSTALL_A).unwrap();
      let app_id = AppId::trusted("com.example.a");
      claim(&db, &install_id, &InstallSecret::from(SECRET_A)).await;
      let first = db
         .enqueue_fcm_message(&install_id, &app_id, Some("a-1"), b"one")
         .await
         .unwrap()
         .unwrap();
      let second = db
         .enqueue_fcm_message(&install_id, &app_id, Some("a-2"), b"two")
         .await
         .unwrap()
         .unwrap();
      assert!(
      ...
      assert!(matches!(
          frames.as_slice(),
          [ServerFrame::Message { id, .. }] if *id == first
      ));

      assert!(matches!(
         connection
            .negative_acknowledge(&install_id, MessageId::new(first.get() + 999))
            .await
            .unwrap(),
         NackOutcome::Invalid
      ));
      assert!(matches!(
         connection
            .negative_acknowledge(&install_id, first)
            .await
            .unwrap(),
         NackOutcome::Deferred
      ));

      // The deferred head must hold back the second message rather than be
      // skipped past.
      assert!(connection.pending_messages().await.unwrap().is_empty());

      db.make_outbox_message_due(first).await.unwrap();
      let frames = connection.pending_messages().await.unwrap();
      assert!(matches!(
          frames.as_slice(),
          [ServerFrame::Message { id, .. }] if *id == first
      ...
      assert!(connection.acknowledge(&install_id, first).await.unwrap());
      let frames = connection.pending_messages().await.unwrap();
      assert!(matches!(
          frames.as_slice(),
          [ServerFrame::Message { id, .. }] if *id == second
      ));
      assert!(connection.acknowledge(&install_id, second).await.unwrap());
      assert!(connection.pending_messages().await.unwrap().is_empty());
   }

   #[tokio::test]
   async fn nack_discards_after_bounded_attempts() {
      let (db, _hub, mut connection) = fresh_connection().await;
      let install_id = InstallId::try_from(INSTALL_A).unwrap();
      let app_id = AppId::trusted("com.example.a");
      claim(&db, &install_id, &InstallSecret::from(SECRET_A)).await;
      ...
      for round in 1..=MAX_DELIVERY_ATTEMPTS {
         ...
         let outcome = connection
            ...
            .await
            .unwrap();
         if round < MAX_DELIVERY_ATTEMPTS {
            assert!(matches!(outcome, NackOutcome::Deferred));
            db.make_outbox_message_due(first).await.unwrap();
         } else {
            assert!(matches!(outcome, NackOutcome::Discarded));
         }
      }
