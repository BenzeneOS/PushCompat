#[path = "gcm_contract.rs"] mod contract;
#[path = "gcm_decimal_string.rs"] mod decimal_string;
...
use crate::{
   ...
   mcs::{
      LoginRequest,
      Setting,
      mod_LoginRequest::AuthService,
   },
};

fn require_some<T>(value: Option<T>, reason: &'static str) -> Result<T, Error> {
   value.map_or_else(
      || Err(Error::DependencyFailure("Android device check-in", reason)),
      Ok,
   )
}
...
const MCS_CONNECT_TIMEOUT: Duration = Duration::from_secs(30);
const MAX_HTTP_RESPONSE_BYTES: usize = 1024 * 1024;
const MAX_DECOMPRESSED_READ_BYTES: u64 = 0x0010_0001;
const MAX_PERSISTENT_IDS: usize = 500;
const MAX_PERSISTENT_ID_BYTES: usize = 4096;

async fn read_response_bytes(mut response: Response, api: &'static str) -> Result<Vec<u8>, Error> {
...
async fn read_response_text(response: Response, api: &'static str) -> Result<String, Error> {
...
pub struct FirebaseRegistrationParams<'a> {
   pub sender_id:             &'a str,
   pub package_name:          &'a str,
   pub cert_sha1:             Option<&'a str>,
   pub app_version:           Option<i32>,
   pub app_version_name:      Option<&'a str>,
   pub target_sdk:            Option<i32>,
   pub firebase_config:       Option<&'a FirebaseConfig>,
   pub firebase_installation: Option<&'a FirebaseInstallation>,
}
...
impl DeviceSessionState {
   ...
   async fn request(
      ...
      let logging_id = i64::from_le_bytes(logging_id_bytes) & i64::MAX;
      ...
      let request = contract::AndroidCheckinRequest {
         ...
         checkin: contract::AndroidCheckinProto {
            ...
            build: Some(contract::AndroidBuildProto {
               ...
               time: Some(1_722_859_200), // Aug 2024
      ...
      let response = http
         ...
         .header(USER_AGENT, user_agent)
            .send()
            .await
            .map_err(|error| Error::Request(API_NAME, error))?;
      ...
      let response = contract::AndroidCheckinResponse::from_reader(&mut reader, &decoded_bytes)
         .map_err(|error| Error::ProtobufDecode("android checkin response", error))?;

      let android_id = require_some(response.android_id, "response is missing android id")?;

      let android_id = i64::try_from(android_id).map_err(|_| {
         Error::DependencyFailure(API_NAME, "responded with non-numeric android id")
      })?;
   ...
   pub(crate) async fn checkin(http: &Client) -> Result<Self, Error> {
   ...
   pub(crate) async fn refresh(&self, http: &Client) -> Result<Self, Error> {
      let mut session =
      ...
      if self.private_key.is_some() {
         session.private_key.clone_from(&self.private_key);
         session.public_key.clone_from(&self.public_key);
         session.auth_secret.clone_from(&self.auth_secret);
      } else {
         session.generate_keys()?;
      }
      Ok(session)
   ...
   /// # Arguments.
   /// * `http` - HTTP client.
   /// * `sender_id` - Firebase sender ID (project number), e.g.,
   ///   "890224420307".
   ...
   pub(crate) async fn register(
      &self,
      http: &Client,
      params: FirebaseRegistrationParams<'_>,
      ...
      let app_ver_str = params.app_version.unwrap_or(1).to_string();
      let target_ver_str = params.target_sdk.unwrap_or(34).to_string();
      let cert_str = params.cert_sha1.map(str::to_lowercase).unwrap_or_default();
      ...
      let form_body = if let (Some(fis), Some(config)) =
         (params.firebase_installation, params.firebase_config)
      {
         format!(
            ...
            urlencoding::encode(params.sender_id),
            ...
            urlencoding::encode(params.app_version_name.unwrap_or("1.0.0")),
      ...
      for attempt in 1..=MAX_RETRIES {
         let result = http
            ...
            .header(USER_AGENT, user_agent)
            .header("app", params.package_name)
   ...
   pub(crate) async fn connect(
      ...
      // The raw TCP connect, TLS handshake, login write, and version byte
      // read would otherwise wait on the TCP retransmission timeout alone.
      timeout(MCS_CONNECT_TIMEOUT, Self::try_connect(domain, &login_bytes))
         .await
         .map_err(|_| Error::DependencyFailure("MCS connection", "connect deadline exceeded"))?
   ...
   fn new_mcs_login_request(&self, received_persistent_id: Vec<String>) -> LoginRequest {
      let android_id = self.android_id.to_string();
      LoginRequest {
         adaptive_heartbeat: Some(false),
         auth_service: Some(AuthService::ANDROID_ID),
   ...
   async fn try_connect(
      ...
      let keepalive = socket2::TcpKeepalive::new()
         .with_time(Duration::from_mins(30))
...
fn new_tls_initiator() -> TlsConnector {
   TlsConnector::from(Arc::new(crate::rustls_client_config()))
}

pub struct Connection(pub TlsStream<TcpStream>);
